...
make.unaids.plots <- function(DT) {
    .aggregate_by_agebins <- function(DT, width = age_group_width) {
        ...
        tmp1 <- merge(dage, DT)
        cols <- c(
            "ROUND", "LOC_LABEL", "SEX_LABEL", "GROUP",
    ...
    plot.unaids.goals <- function(DT) {
        ...
        p <- ggplot(DT, aes(x = GROUP, y = value, group = 1, fill = variable)) +
            ...
            geom_text(
                ...
                x = Inf, y = Inf
            ) + # , vjust=VJUST, hjust=HJUST), x=100, y=100, color='red')
    ...
    ggsave_nature(
        p = p1, filename = filename, LALA = file.path(vl.out.dir, "figures"),
    ...
    ggsave_nature(p2,
        filename = filename, LALA = file.path(vl.out.dir, "figures"),
        w = 18, h = 13, add_reqs = FALSE
    )
    ...
    tmp[, PARTICIPATION_STATUS := gsub(
        "PARTICIPATION_STATUS_",
...
plot_2yaxis_hist_lines <- function(DThist, DTline, sec_name = "Contribution of each age group to PLHIV") {
    ...
    by_cols <- DThist[, sapply(.SD, function(x) typeof(x) == "character")] |>
        which() |>
        names()
...
plot_prevalenceandcontrid <- function(DTprev,
                                      DTcontrib,
    ...
    .plot.facet <- function(DT) {
        ...
        ggplot(DT, aes(x = AGEYRS, y = M, ymin = CL, ymax = CU, fill = LABEL2, color = LABEL2)) +
            ...
            {
                if (CrI) {
                    geom_linerange(position = position_dodge(width = DODGE), linewidth = .2)
                } else {
                    NULL
                }
            } +
...
plot.uganda.map <- function(zoom = "medium", maptype = "toner-lite", position = "corner", labs = TRUE) {
    ...
    position <- match.arg(position, c("corner", "center"))
    ...
    ggmap(uganda) +
        geom_polygon(
            data = map_data("lakes"), aes(x = long, y = lat, group = group), fill = "blue",
            alpha = 0.2
        ) +
...
plot_propofpop_of_viraemic_byagesex_stratbycommround <- function(DT, colorby = "ROUND", cri = TRUE) {
    ...
    .scales <- function(x = { colorby == "ROUND_LAB" }) {
        out <- list(
            ...
            if (x) {
                scale_fill_viridis_d(begin = 0, end = .7, option = "magma", direction = -1)
            },
            if (x) {
                scale_color_viridis_d(begin = 0, end = .7, option = "magma", direction = -1)
            },
            NULL
        )
        out[!sapply(out, is.null)]
