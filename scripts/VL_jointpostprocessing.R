...
out.dir.figures <- file.path(vl.out.dir, 'figures')
out.dir.tables <- file.path(vl.out.dir, 'tables')
...
# load number of census eligible individuals
...
env_list <- store.rda.environments.in.list.by.round.ftpstatus(dfiles_rda[ROUND == 16])
...
p_hiv <- plot.fit.weighted.by.ftpstatus(djoint, "run-gp-supp-hiv")
ggsave2(p=p_hiv, file=filename, LALA=out.dir, w=10, h=12)

filename <- "fit_suppofhiv_byroundcommgender.pdf"
p_supp <- plot.fit.weighted.by.ftpstatus(djoint, "run-gp-prevl")
ggsave2(p=p_supp, file=filename, LALA=out.dir, w=10, h=12)

filename <- "fit_suppofpop_byroundcommgender.pdf"
p_vir <- plot.fit.weighted.by.ftpstatus(djoint, "run-gp-supp-pop")
ggsave2(p=p_vir, file=filename, LALA=out.dir, w=10, h=12)
...
ggsave2(p=p2, file=filename, LALA=out.dir, w=10, h=12)
